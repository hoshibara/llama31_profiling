...
DEVICE = "xpu"
MODEL_NAME = os.environ.get('MODEL_NAME', "meta-llama/Meta-Llama-3.1-70B")  # Default to 8B for profiling
USE_COMPILE = os.environ.get('USE_COMPILE', 'False').lower() in ('1', 'true', 'yes', 'on')  # Set to True for torch.compile
MODEL_CONFIG_NAME = os.environ.get('MODEL_CONFIG_NAME', "4-Func-mail")  # Default to 4-Func for profiling

SHELL_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_CONFIG = LlamaConfig.from_json_file(os.path.join(SHELL_DIR,f'configs/{MODEL_CONFIG_NAME}.json'))
...
MAX_NEW_TOKENS = 5
PROFILE_DIR = os.environ.get('PROFILE_DIR', "/tmp/llama31_profiling")
